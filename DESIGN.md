...
| `utils/elementui-utils.js` | `src/utils/elementui-utils.js` | Element UI + KD 组件 locale 适配 |
...
- 从 Vue 实例化 options 中移除旧的 `i18n` 引用（如果新 elementui-utils 已接管）
...
- 用 `state.repairs.scaffoldRetried` / `injectRetried` 防止循环
- 其他 fail 项则 stop
